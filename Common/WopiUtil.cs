...
namespace inReachWebRebuild.Common
{
    public static class WopiUtil
    {
        ...
        //        using (HttpResponseMessage response = await client.GetAsync(SettingsManager.GetSettingValueAsString("appWopiServer")))
        //        {
        ...
        /// Forms the correct action url for the file and host
        /// </summary>
        public static string GetActionKeyUrl(WopiAction action, FileModel file, string authority)
        {
            // Initialize the urlsrc
            var urlsrc = action.urlsrc;
            ...
            // Add the WOPISrc to the end of the request
            urlsrc += ((phCnt > 0) ? "" : "?") + String.Format("WOPISrc=https://{0}/wopi/files/{1}", authority, file.Key);
            return urlsrc;
        }
